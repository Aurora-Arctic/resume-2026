...
- Two phase workflows: `pr-gate.yml` (PR → `main`) and `merge-queue.yml` (`merge_group`), both composed from small reusable per-check workflows (`lint`, `format`, `typecheck`, `vitest`, `build`, `playwright`, `audit`).
