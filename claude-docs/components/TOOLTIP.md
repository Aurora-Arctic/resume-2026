...
- Deliberately keeps `role="tooltip"` despite APG guidance against nesting an interactive element inside it, per explicit site-owner direction. Axe accessibility audit adds a scoped `nested-interactive` rule exemption (see [claude-docs/ACCESSIBILITY.md](../ACCESSIBILITY.md)) documenting this choice; the pattern remains as-is.
- **Escape-to-dismiss**: pressing Escape while the tooltip is shown (not previously dismissed) dismisses it and persists to `localStorage` — mirrors PrintOptions' Escape-to-close pattern.
