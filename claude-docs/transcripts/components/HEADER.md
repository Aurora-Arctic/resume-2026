...
## Link text replaced with icons (2026-08-10)

The user asked for `&__links`' visible label text (LinkedIn / own-domain / GitHub) to be replaced with icons: the FontAwesome LinkedIn mark, a generic "link" icon for whichever entry matches `OWN_DOMAIN_HOSTNAME`, and the FontAwesome GitHub mark.

**Icon convention, extended to `Header`**: `@fortawesome/fontawesome-free` (devDependency only, never imported as a runtime package/webfont — see `PrintOptions`'s `PrintIcon` and `ThemeToggle`'s hand-built icons) supplied the source path data, hand-transcribed into three local components (`LinkedinIcon`, `GithubIcon`, `LinkIcon`), each a plain `<svg><path fill="currentColor"/></svg>` with a comment citing its `node_modules/@fortawesome/fontawesome-free/svgs/...` source. For LinkedIn, the user chose the bare "in" mark (`brands/linkedin-in.svg`) over the boxed logo (`brands/linkedin.svg`) — the boxed version bakes a background square into its path, which would look inconsistent next to GitHub's and the link icon's plain silhouettes.

**Icon selection**: a new `getLinkIcon(originalHref)` helper sits next to `resolveLink`, inspecting `new URL(originalHref).hostname` — deliberately the _original_, pre-`resolveLink` href, the same input `resolveLink` itself checks against `OWN_DOMAIN_HOSTNAME` — since the own-domain link's _resolved_ hostname varies per environment (`localhost:8000`, `staging.resume.marwynn.net`, ...) and would otherwise break the match. Hostname `=== OWN_DOMAIN_HOSTNAME` maps to `LinkIcon`; anything containing `linkedin.com` to `LinkedinIcon`; `github.com` to `GithubIcon`; anything else falls back to `LinkIcon` as a safety net.

**Accessibility**: each anchor lost its visible text but kept an equivalent accessible name via an explicit `aria-label={resolved.label}` — `Tooltip` (wrapped around each anchor for a hover/focus label, same pattern as `ThemeToggle`/`PrintOptions`) only wires `aria-describedby`, not a name, so it doesn't supply this on its own. This meant no existing `getByRole('link', { name: ... })` assertions needed to change — RTL matches accessible name regardless of whether it comes from text content or `aria-label`. Each `<li>` needed its own `position: relative` (new, previously unstyled) since — unlike `ThemeToggle`/`PrintOptions`, which each mount exactly one `Tooltip` — there are three sibling instances here, each needing its own positioning context so its bubble anchors above its own icon. A new `.resume-header-link-tooltip` placement class follows the established per-instance convention (see `.theme-toggle-tooltip`/`.print-options-tooltip` in their respective `index.scss`).

**Application print tier**: the user separately asked that `&__links` print as a single linear column (one link per line, top to bottom) specifically in the Application print tier, rather than its normal wrapped/right-aligned row. Added a `@include linear-print-tiers { flex-direction: column; align-items: flex-start; }` block, following `Resume/index.scss`'s established pattern of explicitly reasserting Application-specific layout even where it's close to the unstyled default — matching `&__contact`'s existing one-per-line stacking, since that's the same ATS-linear convention Application mode is meant to produce.

**Tests**: `index.test.tsx` gained cases asserting each link's accessible name still resolves to an anchor containing an `svg[aria-hidden="true"]` (icon renders, no visible text), and that the right icon (`viewBox`) is picked per host — LinkedIn, GitHub, own-domain, and an unrecognized host exercising the fallback branch.

## Desktop/print right-alignment removed (2026-08-10)

Companion change to [RESUME.md](RESUME.md)'s transcript entry reverting `.resume`'s Header-right/Summary-left two-column desktop grid — since `Header` no longer occupies a right-hand column at any width, the styling that made sense only for that column also came out: the `@media (min-width: $breakpoint-desktop)`/`grid-print-tiers` `text-align: right` rule on `.resume-header`, and the matching `justify-content: flex-end` rules on `&__links` (both the desktop `@media` and `grid-print-tiers` blocks). `.resume-header` and `&__links` are back to left/start alignment unconditionally, same as mobile always was. The `h1` fixed `font-size: 1.75rem` at `$breakpoint-desktop`/print was left alone — that's a width-cap against `.paper-card`'s 960px ceiling, not something tied to which side the header sits on.
