...
- **Ad-hoc visual verification** (screenshotting/driving the dev server to eyeball a change, outside the `e2e/` suite): see [claude-docs/PLAYWRIGHT-ADHOC-VERIFICATION.md](claude-docs/PLAYWRIGHT-ADHOC-VERIFICATION.md) — there's no `chromium-cli`-style driver in this repo, and a throwaway script must live inside the project tree, not an external scratch directory, or Node's ESM resolution can't find the `playwright` package.
