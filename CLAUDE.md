...
- **PR comments**: `lint`, `format`, `typecheck`, `vitest`, and `playwright` each take a `pr-number` input (comment step only runs if it's supplied) and a `merge-queue` boolean input (default `false`) that switches their PR-comment behavior:
  - From `pr-gate` (`merge-queue: false`): `lint`/`format`/`typecheck` comment only on failure, and a later success minimizes that comment via GitHub's resolve mechanism (`minimizeComment`/`unminimizeComment` GraphQL mutations) rather than leaving it stale. `vitest` always comments with the current pass/fail result, updating the same comment each run. (`playwright` never runs from `pr-gate` today, but supports the same `merge-queue: false` shape for symmetry with `vitest`.)
